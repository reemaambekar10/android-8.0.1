module.exports = (function() {
  ...
  */  var db, root;
  ...
  function Client(opts) {
    ...
    Ti.App.addEventListener('prune_cache', function(){ self.prune_cache(1); });
    ...
    this.xhr.onload = function() {
      //var responseTime=(new Date()) - callTime;
    Ti.API.warn('Call Completed '+ self.options.method + ' ' + self.options.url + (self.options.data ? ' with request body: '+ self.options.data : ''));      
      if(Ti.Platform.osname == 'android') {      	
      	this.clearCookies(this.baseUrl);
      }
    ...
    this.xhr.onerror = function() {
      //var responseTime=(new Date()) - callTime;
      Ti.API.error('Calling '+ self.options.method + ' ' + self.options.url + (self.options.data ? ' with request body: ' + self.options.data : ''));
      // Ti.API.error('Response : ' + this.responseText);
      log("ERROR");
      var Core=require('core');
      //If the app is in the inner screen and the password is Ti.UI.iPhone.TableViewSeparatorStylechanged from backend then redirect the user to login screen.
      if(this.status===401 && Core.isLoggedIn()){
          Core.logoutUser();
          alert('Your session has expired. Please login again.');
      }else{
      	console.log("error message "+this.responseText +" "  +this.status);
    ...
    if (response = this._get_cached_response()) {
      ...
      return this.onload_hook(this, response);
       ...
       this._setRequestHeaders();
		    Ti.API.warn('Calling '+ self.options.method + ' ' + self.options.url + (self.options.data ? ' with request body: '+ self.options.data : ''));
  ...
  Client.prototype.onload_hook = function(self, response) {
    ...
    if (this.options.onload != null) {
      ...
      Ti.API.error("Client: Please specify an onload callback!");
    }
  ...
  Client.prototype.onerror_hook = function(self, response) {
    ...
    if (this.options.onerror != null) {
      if(this.options.method !== "POST") {
      	var cData = this._get_cached_response(9999999) ;
      	response = cData ? cData : response;
     	}
